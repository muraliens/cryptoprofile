...
void sm4_keystream(uint8_t key[], uint8_t iv[], int length, uint8_t keystream[])
{
	uint8_t cp[SM4_BLOCK_SIZE];
	int offset = 0;
	uint32_t cnt;
	uint32_t *counter = (uint32_t *)(iv + 12);
	*counter = 0;
	while (length > 0)
	{
		sm4_encrypt(key, iv, cp);
		if (length > SM4_BLOCK_SIZE)
		{
			memcpy(keystream+offset, cp, SM4_BLOCK_SIZE);
		}
		else
		{
			memcpy(keystream+offset, cp, length);
		}
		cnt = *counter;
		cnt = SWAP32(cnt);
		cnt++;
		cnt= SWAP32(cnt);
		*counter = cnt;
		offset = offset + SM4_BLOCK_SIZE;
		length = length - SM4_BLOCK_SIZE;
	}
}
*/
import "C"
...
func (h *Handle) SM4Stream() cryptoprofile.BitStream {
	if len(h.key) == 0 {
		h.key = make([]byte, 16)
		rand.Read(h.key)
	}
	...
	C.sm4_keystream(C.BytePtr(key), C.BytePtr(iv), C.int(numberStream), C.BytePtr(keystreams))
